...
__unused static struct slab *alloc_slabmgt() { return NULL; }
...
void kmem_cache_init() __init {
  ...
  for (uint32_t kmalloc_type = KMALLOC_NORMAL; kmalloc_type < NR_KMALLOC_TYPES;
       ++kmalloc_type) {
    for (uint32_t i = 0; i < NR_KMALLOC_CACHES; ++i) {
      ...
      if (!off_slab(kmalloc_cache)) {
        off_slab_obj_limit = (kmalloc_cache->obj_size - sizeof(struct slab)) /
...
kmem_cache_create(const char *name, uint64_t size, uint64_t align,
  ...
  if (!name || (flags & (~SLAB_ALLOWD_FLAGS)) || (!pow_of_2(align) && align)) {
  ...
  uint32_t obj_num = 0;
  do {
    cache_estimate(cachep->order, cachep->obj_size, align, flags, &left_over,
                   &slab_size, &obj_num);
    if (!obj_num) {
      ++cachep->order;
    }
